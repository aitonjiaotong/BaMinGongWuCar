...
public class AddressAdapter extends ArrayAdapter<GeoPointLable> {
	...
	public GeoPointLable getGeo(int position, String name) {
		return new GeoPointLable(getItem(position).getLat(), getItem(position).getLog(), name, getItem(position).getCityName());
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		SysDeug.logD(getItem(position).getName() +"----<<< AddressAdapter" + getItem(position).getCityName()+ "===>GeoPointLable--->" + gp.toString());
		text.setText(getItem(position).getName() + "(" + getItem(position).getCityName() + ")");
