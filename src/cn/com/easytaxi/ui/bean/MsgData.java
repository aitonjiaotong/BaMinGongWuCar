...
public class MsgData extends BaseData<MsgBean> {
	 ...
	 * @return List<MsgBean>
	 */
	private List<MsgBean> getMsgListRemote(int start, String cityId, String passengerId,int mtype) {
...
//			}
	 ...
	 * @return
	 */
	public void getMsgList(final int start, final String cityId, final String passengerId, final int type, LoadCallback<List<MsgBean>> callback) {
		AsyncUtil.goAsync(new Callable<List<MsgBean>>() {

			@Override
			public List<MsgBean> call() {
				List<MsgBean> msgs = getMsgListRemote(start, cityId, passengerId,type);
				return msgs;
			}
		}, callback);
