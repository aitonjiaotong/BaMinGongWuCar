...
public class BookFragementActivity extends BaseFragementActivity implements ViewPager.OnPageChangeListener {
	...
	private int startLat, startLng;
	private String address, startCity;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// showRegistDialog(getBaseContext(), "提示", "请先注册以后再使用此功能", "立刻注册",
		// "稍后再说", "cn.com.easytaxi.platform.RegisterActivity");
		// }
	}
	...
	private void initListeners() {
		...
		// });
		//
		// radioGroup_choice.setOnCheckedChangeListener(new
	...
	private void initViews(Bundle savedInstanceState) {
		...
		titleBar.setTitleName("预约用车");
		titleBar.getRightHomeButton().setText("计费规则");
		titleBar.getRightHomeButton().setVisibility(View.VISIBLE);
		titleBar.setRightCallback(new Callback<Object>() {

			@Override
			public void handle(Object param) {
				Intent intent = new Intent(BookFragementActivity.this, MyWebViewActivity.class);
				intent.putExtra("title", "计费规则");
				intent.putExtra("url", PayActivity.PRICE_RULE + LocationBroadcastReceiver.getcity());
		...
		startLat = getIntent().getIntExtra("startLat", 0);
		startLng = getIntent().getIntExtra("startLng", 0);
		startCity = getIntent().getStringExtra("city");
		address = getIntent().getStringExtra("address");

		bookPublishFragment = new BookPublishFragement();
		bookPublishFragment.setInit(startLat, startLng, address, startCity, getIntent().getIntExtra("carType", 1));
