...
public class BookShow extends BaseBookLoader {
	...
	private void setData() {
		...
		try {
			...
			if (!TextUtils.isEmpty(currentBook.getForecastDistance()) && !TextUtils.isEmpty(currentBook.getForecastPrice())) {
				String subDistance = currentBook.getForecastDistance();
					...
					try {
						if (param != null) {
							...
							if (result.getInt("id") != id) {
								ToastUtil.show(self, "取消失败");
							} else {
								ToastUtil.show(self, "取消成功");
								changeOptState(0);
