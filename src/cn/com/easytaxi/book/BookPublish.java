			...
			try {
				...
				try {
					if (use.getTimeInMillis() <= now.getTimeInMillis()) {
						selectDateByWheel();
						ToastUtil.show(self, "用车时间必须大于当前时间");
						return;
					}
					if (use.getTimeInMillis() >= limit.getTimeInMillis()) {
						selectDateByWheel();
						ToastUtil.show(self, "用车时间必须为今天或明天");
						return;
					}
				} catch (Exception e) {
					return;
				...
				if (strStartAddr.equals("")) {
					ToastUtil.show(self, "请输入上车地点");
					etStartAddr.requestFocus();
					return;
				}
				if (strEndAddr.equals("")) {
					ToastUtil.show(self, "请输入下车地点");
	...
	*/}
	...
	private class AddressSetBroadCast extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent data) {
			...
			if (point == null) {
				ToastUtil.show(BookPublish.this, "未选择地点");
				return;
			}
			if (who == btnStartLoc.getId()) {
				...
				etEndAddr.setTag(new GeoPoint(point.getLat(),  point.getLog()));
			}
			setDistance();
		}
	}
